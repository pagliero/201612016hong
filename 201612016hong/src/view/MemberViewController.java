...
private String ageCheck() { 
		...
		int age1 = Integer.parseInt(tfBirth.getText().substring(0,4));  // 입력된 년도 
		int age2 = Integer.parseInt(tfBirth.getText().substring(4,8));  //입력된 월, 일 
		int year = cal.get(Calendar.YEAR);
		int month = (cal.get(Calendar.MONTH)+1)*100;  // 현재 월 뽑아서 100곱해서  
		int date = cal.get(Calendar.DATE);  // 현재 일 
		int DB = month + date;  // 현재 월,일 합침  
	
		if ( age2 >= DB  ) {   // 생일 지남 
			
			return Integer.toString(year - age1-1);
		} else {    // 생일 안 지남 
			return Integer.toString(year - age1); 
		}
